...
func VideoPost(video model.Video) error {
	...
	// MySQLの操作
	_, err = tx.Exec("INSERT INTO video (id, title, url, user_id, birth_time, update_time, public) VALUES (?, ?, ?, ?, ?, ?, ?)", video.Id, video.Title, "", video.UserId, video.BirthTime, video.UpdateTime, video.Public)
	if err != nil {
...
func VideoGet(id string) (model.Video, error) {
	...
	if rows.Next() {
		...
		// クエリ結果が空の場合
		return model.Video{}, errors.New("Couldn't find the movie specified")
	}
