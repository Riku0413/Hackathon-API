...
func VideosGetHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodGet:
		...
		if err != nil {
			...
			log.Printf("fail: controller.VideosGetHandler(), %v\n", err)
			return
		}
		var user model.User
		...
		videos, err := usecase.VideosGet(user.Id)
		if err != nil {
			log.Printf("fail: usecase.VideosGet(), %v\n", err)
			log.Printf("fail: controller.VideosGetHandler(), %v\n", err)
		...
		log.Printf("get: controller.VideosGetHandler()")

	default:
		log.Printf("fail: controller.VideosGetHandler(), HTTP Method is %s\n", r.Method)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
}
