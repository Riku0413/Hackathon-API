...
func BookPutHandler(w http.ResponseWriter, r *http.Request) {
	...
	case http.MethodPut:
		...
		if err := decoder.Decode(&book); err != nil {
			...
			log.Printf("fail: controller.BookPutHandler(), %v\n", err)
		...
		if err := usecase.BookPut(book); err != nil {
			log.Printf("fail: usecase.BookPut(), %v\n", err)
			log.Printf("fail: controller.BookPutHandler(), %v\n", err)
		...
		log.Printf("put: controller.BookPutHandler(), %v\n", err)

	default:
		log.Printf("fail: controller.BookPutHandler(), HTTP Method is %s\n", r.Method)
		w.WriteHeader(http.StatusBadRequest)
		return
	}
}
